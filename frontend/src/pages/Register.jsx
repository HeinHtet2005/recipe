...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
   ...
   try {
    ...
    const response = await axios.post('http://localhost:4000/api/users/register',user,{withCredentials:true});
          ...
          <form action="#" method="POST" className="space-y-6" onSubmit={handleSubmit}>
            <div>
              ...
              <div className="mt-2">
                ...
                />
                {!!(error && error.name)&&<p className="text-red-500 text-md">{error.name.msg}</p>}
              </div>
            </div>
            <div>
              <label
              ...
              <div className="mt-2">
                <input
                  ...
                  className="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-orange-600 sm:text-sm/6"
                />
                {!!(error && error.email)&&<p className="text-red-500 text-md">{error.email.msg}</p>}
              </div>
            </div>
            ...
            <div>
             ...
             {!!(error && error.password)&&<p className="text-red-500 text-md">{error.password.msg}</p>}
            <div>
              <button
                type="submit"
